...
namespace InvestmentPortfolio
{
 public partial class InvestmentDialog : Window
 {
  ...
  {
   decimal q,p;if(string.IsNullOrWhiteSpace(NameBox.Text)||!decimal.TryParse(QuantityBox.Text.Replace(',','.'),NumberStyles.Any,CultureInfo.InvariantCulture,out q)||!decimal.TryParse(PurchaseBox.Text.Replace(',','.'),NumberStyles.Any,CultureInfo.InvariantCulture,out p)||TypeBox.SelectedItem==null||CurrencyBox.SelectedItem==null){MessageBox.Show("Uzupełnij poprawnie wymagane pola.","Błąd",MessageBoxButton.OK,MessageBoxImage.Warning);return;}
   Investment.Name=NameBox.Text.Trim();Investment.Ticker=TickerBox.Text.Trim();Investment.Type=(InvestmentType)TypeBox.SelectedItem;Investment.Currency=CurrencyBox.SelectedItem.ToString();Investment.Quantity=q;Investment.PurchasePrice=p;if(Investment.CurrentPrice<=0)Investment.CurrentPrice=p;Investment.PurchaseDate=DateBox.SelectedDate??DateTime.Today;DialogResult=true;
  }
