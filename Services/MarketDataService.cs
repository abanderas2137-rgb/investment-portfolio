 ...
 public class MarketDataService
 {
  ...
  public async Task<MarketQuote> GetQuoteAsync(string symbol)
  {
   ...
   var url = "https://query1.finance.yahoo.com/v8/finance/chart/" +
             Uri.EscapeDataString(symbol.Trim()) + "?range=1d&interval=1d";
   ...
   using(var request = new HttpRequestMessage(HttpMethod.Get, url))
   {
    ...
    if(price == null)
     throw new InvalidOperationException("Brak ceny dla symbolu " + symbol);
    ...
    return new MarketQuote
    {
     Symbol = symbol,
     Price = price.Value,
     Timestamp = ts.HasValue
       ? DateTimeOffset.FromUnixTimeSeconds(ts.Value).LocalDateTime
       : DateTime.Now,
  ...
  private static long? ExtractLong(string json, string key)
  {
   ...
   long v;
   return m.Success && long.TryParse(m.Groups[1].Value, out v)
     ? (long?)v : null;
  }
 }
}